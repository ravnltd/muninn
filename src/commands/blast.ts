...
import type { BlastSummary, BlastResult, BlastComputeOptions } from "../types";
...
function computeTransitiveDependents(
  ...
  while (queue.length > 0) {
    ...
    for (const next of nextDependents) {
      if (!affected.has(next) || affected.get(next)!.distance > current.distance + 1) {
        queue.push({
          file: next,
...
function storeBlastRadius(
  ...
  db.run(
    `DELETE FROM blast_radius WHERE project_id = ? AND source_file = ?`,
...
function storeBlastSummary(
  ...
  const blastScore = calculateBlastScore(
    totalAffected,
    testCount,
    routeCount,
    result.maxDepth
...
export function getBlastRadius(
  ...
  if (!forceRefresh) {
    const cached = db.query<BlastSummary, [number, string]>(`
...
export function getHighImpactFiles(
  ...
  return db.query<BlastSummary, [number, number]>(`
 ...
 */
export function showBlastRadius(
  db: Database,
  projectId: number,
  projectPath: string,
  filePath: string
 ...
 */
export function showHighImpactFiles(
  db: Database,
  projectId: number
): void {
  ...
  for (const file of files) {
    ...
    console.error(`   Score: ${file.blast_score.toFixed(1)} | Affected: ${file.total_affected} | Tests: ${file.affected_tests} | Routes: ${file.affected_routes}`);
  }
  console.error("");
