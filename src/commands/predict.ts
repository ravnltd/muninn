...
function searchForTask(
  ...
  try {
    result.decisions = db
      .query<{ id: number; title: string }, [string, number]>(`
  ...
  try {
    result.issues = db
      .query<{ id: number; title: string; severity: number }, [string, number]>(`
  ...
  try {
    result.learnings = db
      .query<{ id: number; title: string; content: string }, [string, number]>(`
...
function searchRelatedFiles(
  ...
  try {
    const files = db
      .query<{ path: string; purpose: string | null }, [string, number]>(`
...
function getIssuesForFiles(
  ...
  try {
    ...
    for (const file of files) {
      const issues = db
        .query<{ id: number; title: string; severity: number }, [number, string]>(`
...
function getApplicableWorkflow(
  ...
  try {
    const workflow = db
      .query<{ task_type: string; approach: string }, [number, string]>(`
 ...
 */
function getLastSessionContext(db: Database, projectId: number): PredictionBundle["lastSessionContext"] {
  try {
    // Get last completed session
    const lastSession = db
      .query<{ id: number; goal: string | null }, [number]>(`
