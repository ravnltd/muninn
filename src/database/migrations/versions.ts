 ...
 * Each migration version is atomic and validated.
 */
...
export const MIGRATIONS: Migration[] = [
  ...
  // Version 43: v8 — Add duration_ms to work_queue for job instrumentation
  {
    version: 43,
    name: "worker_job_duration",
    description: "Track job execution duration for observability",
    up: `
      ALTER TABLE work_queue ADD COLUMN duration_ms INTEGER;
    `,
    validate: (db) => {
      const cols = db
        .query<{ name: string }, []>("PRAGMA table_info(work_queue)")
        .all();
